...
namespace TowerHaven
{
    ...
    /// </summary>
    public partial class EndlessMode : Window
    {
        # region constants
        ...
        public EndlessMode()
        {
            ...
            level.LoadCampaign("Endless Mode");
            this.level = level;
        ...
        private void LoadMoves()
        {
            ...
            foreach (SpawnPoint s in level.GetWave(wave - 1).spawns)
                moves = pathing.GetMoves(s.Start, s.End);
        }
                        ...
                        for (int j = 0; j < moves.Length; ++j)
                        {
                            ...
                            for (int i = 0; i < moves.Length; ++i)
                            {
                                if (moves[i][0] == x && moves[i][1] == y)
                                {
                                    int waveId = 0;
                                    int temp = j;
                                    while (temp >= level.GetWave(wave - 1).spawns.Count)
                                    {
                                        temp -= level.GetWave(wave - 1).spawns.Count;
                                        waveId++;
                                    }
                                    if (i == 0)
                                        pathing.GetMoves(level.GetWave(wave - 1).spawns[temp].Start, level.GetWave(wave - 1).spawns[temp].End, x, y);
                                    else
                                        pathing.GetMoves(moves[i - 1], moves[i + 1], x, y);
                                }
                            }
                            if (done)
                                break;
                ...
                if (enemy.CanMove)
                {
                    ...
                    else
                    {
                        ...
                        if (enemy.move > 1 || fear == 1)
                            moveSpot = moves[moves.Length - fear - enemy.move];
                        else 
